...
class Agent:
    ...
    def train(self, buffer1, buffer2):
        qloss = 0.0
        if minibatch_size <= len(buffer1) and minibatch_size <= len(buffer2):
            #smaple a random minibatch
            minibatch1 = np.array(random.sample(buffer1, minibatch_size))
            minibatch2 = np.array(random.sample(buffer2, minibatch_size))
